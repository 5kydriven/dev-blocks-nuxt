	...
	const optionsValues = ref(
		props.options?.reduce(
			(acc, option) => {
				...
				if (
					option.name.toLowerCase().endsWith('color') &&
					option.items?.length
				) {
				...
				>
					...
					>
						...
						<UFormField
							...
							:ui="{
								wrapper:
									'bg-elevated/50 rounded-l-sm flex border-r border-accented',
					...
					<iframe
						...
						:class="[
							props.class,
							{ 'dark:bg-neutral-950/50 rounded-t-md': props.elevated },
							!iframeMobile && 'lg:left-1/2 lg:-translate-x-1/2 lg:w-[1024px]',
						]"
					/>
					<div
						v-else
